...
class RBM:
    def __init__(self, nvis, nhid):
        ...
        self.W = np.random.rand(nvis + 1, nhid + 1) / 10
        self.dW = np.random.rand(nvis + 1, nhid + 1) / 1000
    ...
    def genData(self):
        ...
        N = 20 * self.Ndat
        X = array(np.random.random(N))
        Y = array(np.random.random(N))
    ...
    def activ(self, who):
        if (who == 0):
            self.Svis = dot(self.W, self.Shid) + self.sig * np.random.randn(self.nvis + 1)
            self.Svis = self.sigFun(self.Svis, self.Avis)
            self.Svis[-1] = 1.0  # bias
        if (who == 1):
            self.Shid = dot(self.Svis, self.W) + self.sig * np.random.randn(self.nhid + 1)
    ...
    def learn(self, epochmax):
        ...
        for epoch in range(1, epochmax):
            ...
            for point in range(self.Ndat):
                ...
                for recstep in range(int(ksteps)):
                    self.activ(0)
                    self.activ(1)
